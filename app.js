...
function buildChip(type, name, value, isUser, fsid) {
    ...
    // Use a real input but handle selection via click on the div, not label wrapping
...
<button type="button" class="btn-chip-edit" title="Edit" onclick="editChip(event,this)">${SVG_EDIT}</button>
    ...
    // Toggle selection on click (anywhere except the action buttons)
...
function toFieldKey(str) {
 return String(str).replace(/[~*[\]]/g, "_").replace(/\//g, "|");
}
        ...
        existingMap.forEach((stored, qid) => {
            ...
            if (isCards) {
                ...
                group.querySelectorAll(".chip").forEach(chip => {
                    ...
                    if (!inp || !span) return;
                    if (span.getAttribute("contenteditable") === "true") return;
                    const origVal = chip.dataset.origVal || inp.value;
                    if (removed.has(origVal)) { chip.remove(); return; }
                    const newLabel = renamed[toFieldKey(origVal)];
...
window.showResults = function () {
  ...
  (Array.isArray(vals) ? vals : [vals]).forEach(v => { if (v) liveCounts[key][v] = (liveCounts[key][v] || 0) + 1; });
            ...
            if (d.dynamicAnswers) {
                Object.entries(d.dynamicAnswers).forEach(([qTitle, val]) => inc("dyn_" + qTitle, val));
            }
        });
...
function renderTextList(title, key, content) {
    const entries = Object.keys(liveCounts[key] || {}).filter(v => v.trim());
    if (!entries.length) return;
    const block = document.createElement("div");
    block.className = "result-block";
    block.innerHTML = "<h4>" + escHtml(title) + "</h4>";
    entries.forEach(text => {
        block.innerHTML += `<div class="text-response">* ${escHtml(text)}</div>`;
    });
